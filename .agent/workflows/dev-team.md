...
4. 若目前在 downstream / 新專案工作區，且存在 Obsidian access surface，還必須先依 `AGENT_ENTRY.md` 完成 `Obsidian Knowledge Intake Gate`：先檢閱 `00-indexes/`，再受控讀取最小必要的 `20-reviewed/` 文件；不得在啟動階段掃描其他 vault 區域。
