#!/usr/bin/env python3
"""Fallback bridge Python client (localhost-only helper).

This client talks to the local fallback bridge HTTP server.

Today that server may be provided either by:
- the on-demand fallback extension bridge, or
- a legacy compatibility bridge kept alive during migration.
...
Examples:
    ...
    python .agent/runtime/scripts/sendtext_bridge_client.py send --terminal-kind codex --text "hi" --submit

Note:
- this is not the PTY primary path; use it only when workflow has already switched to fallback mode.
- /healthz and /send are the only compatibility surface this client intentionally keeps.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Fallback bridge Python client")
    ...
    p_send.add_argument("--terminal-kind", required=True, choices=["codex", "copilot"])
