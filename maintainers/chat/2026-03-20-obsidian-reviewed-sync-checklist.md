...
原則是：讀取預設放在 read-only mount；downstream default 的寫入只集中到 `10-inbox/pending-review-notes/` 這個單一 writable inbox zone，而且這個區域不屬於啟動前置閱讀面，只在 capture / triage 命中時 on-demand read。`10-inbox/reviewed-sync-candidates/` 對 downstream 預設一律不讀不寫。
